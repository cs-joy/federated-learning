...
class Block:
    ...
    # is False by default, used for pow and verification, in which pow_prooof has to be None, because at this moment - 
    # pow - block hash is None, so does not affect much
    # verification - the block already has its hash
    # if hash_entire_block == True -> used in set_previous_block_hash, where we need to hash the previous whole previous block
    ...
    def return_miner_rsa_public_key(self):
    ...
    def add_verified_transaction(self, transaction):
        ...
        # transactions can be both worker's or validator's transactions
