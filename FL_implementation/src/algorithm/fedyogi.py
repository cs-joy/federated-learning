...
class FedyogiOptimizer(BaseOptimizer, Optimizer):
    ...
    def accumulate(self, mixing_coefficient, local_layer_iterator, check_if= lambda name: 'num_batches_tracked' in name):
        for group in self.param_groups:
            for server_param, (name , local_signals) in zip(group['params'], local_layer_iterator):
                if check_if(name):
