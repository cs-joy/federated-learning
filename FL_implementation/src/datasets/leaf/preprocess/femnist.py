...
def preprocess(root):
    MAX_WRITERS = 100   # max numer of writers per json files
    ...
    def _parse_files(path):
        def _parse_class_files(path):
            ...
            for cl in classes:
                ...
                for subcl in subcls:
                    ...
                    image_dirs = [os.path.join(rel_class_dir, i) for i in images]
        ...
        def _parse_write_files(path):
            write_files = []    # (write, file directory)
        ...
        # parse class and write files sequentially
    ...
    def _match_by_hashes(path):
        ...
        for tup in write_file_hashes:
            w, f, h = tup
            write_class.append((w, f, class_hash_dict[h][0]))
        wwcd = os.path.join(path, 'íntermediate', 'write_with_class')  
        _save_obj(write_class, wwcd)

    def _group_by_write(path):
            wwcd = os.path.join(path, 'intermediate', 'write_with_class')
            write_class = _load_obj(wwcd)

            writers, cimages = [], []   # each entry is a (write, [list of (file, class)]) tuple
            cw, _,  = write_class[0]
    ...
    def _convert_to_json(path):
        def _relable_fmnist_class(c):
            """
            Maps hexadecimal class value (string) to a decimal numbers.
            ...
            Returns:
                ...
                - 36 through 61 for classes reperesenting respectivew lowercase letters
            """
    ...
    # parse files
    logger.info(f'[LOAD] [LEAF - {DATASET_NAME.upper()}] Extract files for raw images...!')
    _parse_files(path)
    logger.info(f'[LOAD] [LEAF -{DATASET_NAME.upper()}] ...finished extracting files of raw images!')

    # get files hashes
    logger.info(f'[LOAD] [LEAF - {DATASET_NAME.upper()}] Calculating image hashes...!')
    _get_hashes(path)
    ...
    logger.info(f'[LOAD] [LEAF - {DATASET_NAME.upper()}] ...finished converting data json format!')