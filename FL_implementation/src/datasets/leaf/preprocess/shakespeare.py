...
def preprocess(root):
    ...
    # `The comedy of Errors` has errors in it's indentation so we need to use different regular expressions
    ...
    def _split_into_plays(shakespeare_full):
        ...
        for i, line in enumerate(slines):
            ...
            if 'by William Shakespeare' in line:    # this is a pretty good heuristic for detecting the start of a new play
                ...
                # this title will be 2, 3, 4, 5, 6, or 7 lines above "by William Shakespeare".
                if slines[i - 2].strip():
                    title = slines[i - 2]
            ...
            if match:
                ...
                Note that this was not done in the original pre-processing and is a recent fix.
                """
                ...
                if match:
                    if comedy_of_errors and match.group(1).startswith('<'):
                        ...
                        character[current_character].append(match.group(1))
                        continue
            
            line = line.strip() # didn't consume the line
            if line and i > 2646:   # before 2646 are the sonnets, which expect to discard.
    ...
    def _get_all_examples(plays):
        ...
        for play, characters in plays:
            ...
            for character, sound_bites in characters.items():
                ...
                if len(examples) <= 2:
                    skipped_characters += 1 # skip characters with fewer than 2 lines since we need at least one train and test line
                    continue
    ...
    def _write_data(examples, output_dir):
        """
        Writes a collect of data files composed of play and character.
        """
    ...
    def _convert_to_json(path):
        ...
        def _to_data(txt_path, seq_length= 80):
            ...
            for i in range(0, len(raw_text) - seq_length, 1):
                seq_in = tokenized[i:1 + seq_length]
    ...
    path = os.path.join(os.path.expanduser(root), 'shakespear')
