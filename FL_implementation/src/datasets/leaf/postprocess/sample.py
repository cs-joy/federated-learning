...
def sample_clients(dataset_name, root, seed, used_raw_data_fraction):
    ...
    for f in files:
        ...
        num_new_samples = int(used_raw_data_fraction * total_num_samples)
        hierarchies = None

        # non-IID split
        client_total_num_samples = 0
        users = data['users']
        ...
        while(client_total_num_samples < num_new_samples):
            hierarchy = None
            ...
            client_data = data['user_data'][user]
            client_num_samples = len(data['user_data'][user]['y'])

            if (client_total_num_samples + client_num_samples) > num_new_samples:
                client_num_samples = num_new_samples - client_total_num_samples
                indices = [i for i in range(client_num_samples)]
                new_indices = rng.sample(indices, client_num_samples)
                x, y = [], []
            ...
            num_samples.append(client_num_samples)
            user_data[user] = client_data
            client_total_num_samples += client_num_samples
            user_i += 1
