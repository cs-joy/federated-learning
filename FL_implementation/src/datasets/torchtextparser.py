...
def fetch_torchtext_dataset(args, dataset_name, root, tokenizer, seq_len, num_embeddings):
    ...
    logger.info(f'[LOAD] [{dataset_name.upper()}] ...successfully extracted archive files!')
    ...
    # tokenize training and test data and prepare inputs/targets
    logger.info(f'[LOAD] [{dataset_name.upper()}] Creating training and test set!')
    if tokenizer is None:
    ...
    logger.info(f'[LOAD [{dataset_name.upper()}] ...created training and test set!')
