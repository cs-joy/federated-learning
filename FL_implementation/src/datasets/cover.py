...
class Cover(Dataset):
    ...
    def __getitem__(self, index):
        inputs, targets = torch.tensor(self.inputs[index]).flotat(), torch.tensor(self.targets[index]).long()
...
def fetch_cover(args, root, seed, test_size):
    ...
    logger.info(f'[LOAD] [COVER] Check if raw data exists; if not, start download!')
    if not os.path.exists(os.path.join(root, 'covtype')):
        _download(root= os.path.join(root, 'covtype'))
    ...
    client_datasets = _munge_and_split(os.path.join(root, 'covtype'), seed, test_size)
    logger.info(f'[LOAD] [COVER] ...munged and splitted dataset!')
