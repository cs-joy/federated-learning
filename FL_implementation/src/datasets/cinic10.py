...
class CINIC10(ImageFolder):
    ...
    def __init__(self, root, split='train', download= True, transform= None, **kwargs):
        ...
        self.split = torchvision.datasets.utils.verify_str_args(split, 'split', self.splits)
        if download:
            self.download()
        
        if not self._check_exists():
            err = 'Dataset not found or corrupted. You can download=True to download it!'
    ...
    def __repr__(self):
        repr_str = {'train': 'CLIENT', 'test': 'SERVER'}
        return f'[CINIC10] {repr_str[self.split]}'
