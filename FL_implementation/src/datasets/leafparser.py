...
def fetch_leaf(args, dataset_name, root, seed, raw_data_fraction ,test_size, transforms):
    CONFIG = {
        'fmnist': {'in_channels': 1, 'num_classes': 62},
        'shakespeare': {'num_embeddings': 80, 'num_classes': 80},
        'sent_140': {'num_embeddings': 400000 + 1 if args.model_name == 'Sent140LSTM' else None, 'seq_len': 25, 'num_classes': 2},   # using GloVe 300-dim embeddings
        'celeba': {'in_channels': 3, 'img_path': f'{root}/celeba/raw/img_align_celeba', 'num_classes': 2},
        'reddit': {'num_embeddings': 10000, 'seq_len': 10, 'num_classes': 10000}    # + 1 for unknown token
    }
    ...
    def _assign_to_clients(dataset_name, dataset_class, raw_train, raw_test, transforms):
        def _construct_dataset(idx, user):
            ...
            te_dset.transforms = transforms[1]
    ...
    logger.info(f'[LOAD] [LEAF - {dataset_name.upper()}] Post-process raw data to be split into train and test!')
    args.num_clients = postprocess.postprocess_leaf(dataset_name.lower(), root, seed, raw_data_fraction=raw_data_fraction, min_samples_per_clients= 0, test_size= test_size)
    logger.info(f'[LOAD] [LEAF - {dataset_name.upper()}] done post-processing raw data into train and test splits!')

    # get raw data
    logger.info(f'[LOAD] [LEAF - {dataset_name.upper()}] Load training and test datasets...!')
    ...
    logger.info(f'[LOAD] [LEAF - {dataset_name.upper()}] ...done parsing training and test datasets!')
