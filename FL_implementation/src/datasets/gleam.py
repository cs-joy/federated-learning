...
def fetch_gleam(args, root, seed, test_size, seq_len):
    ...
    def _munge_and_split(root, seed, test_size):
        ...
        for idx, (path, dirs, files) in enumerate(os.walk(root)):
            if idx == 0:    # demographic information
                ...
                # assign activity
                sensor['Activity'] = sensor.apply(lambda x: assign_acitivity(x, ref_dict), axis= 1)
                ...
                raw = pd.concat(
                    [
                        sensor.iloc[:, 1:-1],   # features (x, y, z sensors)
                ...
                # re-assign padded inputs: i.e., (num_chunks, seq_len, in_fetures)
                train_inputs, train_targets = np.array(train_inputs_chunks).astype(float), np.array(train_targets_chunks).astype(int)
                test_inputs, test_targets = np.array(test_targets_chunks).astype(float), np.array(test_targets_chunks).astype(int)

                # assign as a client dataset
                clients_datasets.append(
                    (
    ...
    logger.info(f'[LOAD] [GLEAM] ...munged and splitted dataset!')
