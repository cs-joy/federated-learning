...
def fetch_adult(args, root, seed, test_size):
    ...
    logger.info(f'[LOAD] [ADULT] Mungin dataset and create clients!')
    raw_clients = _munge_and_create_clients(os.path.join(root, 'adult'))
    logger.info('[LOAD] [ADULT] ...munged dataset and created clients!!')

    logger.info(f'[LOAD] [ADULT] Processing client datasets!')
    clients_datasets = []
    for dataset in raw_clients.values():
        clients_datasets.append(_process_client_datasets(dataset, seed, test_size))
