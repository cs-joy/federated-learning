...
# helper method to fetch dataset from `torchvision.dataset`
def fetch_torchvision_dataset(args, dataset_name, root, transforms):
    logger.info(f'[LOAD] [{dataset_name.upper()}] Fetching datasets!')
    ...
    if dataset_name in [
        ...
        # configure argumenets for taining/test dataset
        train_args = DEFAULT_ARGS.copy()
        ...
        # NOTE: these datasets do NOT provide pre-defined splits
        ...
        # set raw datasets (no holdout set us supported)
        raw_train = VisionClassificationDataset(raw_train, dataset_name.upper(), 'CLIENT')
        raw_test = None
